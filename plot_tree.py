...
def plot_rrt_graph(rrt):
    fig, ax = plt.subplots()
    # Plot start and end points
    ax.plot(rrt.start[0], rrt.start[1], "ro", label="Start") # Start in red
    ax.plot(rrt.end[0], rrt.end[1], "go", label="End") # End in green

    for node in rrt.node_list:
        if node.parent:
            # Plot line from parent node to current node
            plt.plot([node.coords[0], node.parent.coords[0]], 
                     [node.coords[1], node.parent.coords[1]], 
                     "b-") # Blue lines for paths

    # If you have obstacles, plot them here. Assuming obstacles are a list of [x, y] coordinates
    for obstacle in obj_list:
        ax.plot(obstacle[0], obstacle[1], "") # Obstacles in red squares

    # If you want to plot the current position of the robot
    # ax.plot(current_robot_position[0], current_robot_position[1], "ko", label="Robot") # Robot in black
...
def get_main_nodes():
    ...
    #plot the tree taken
    node_tree_list = rrt.node_list
    plot_rrt_graph(rrt)
    return obtained_path,cond

def plot_rrt_graph(rrt, obj_list=obj_list):
    fig, ax = plt.subplots()
    # Plot start and end points
    ax.plot(rrt.start[0], rrt.start[1], "ro", label="Start")  # Start in red
    ax.plot(rrt.end[0], rrt.end[1], "go", label="End")  # End in green
    ...
    # Plot obstacles
    for obj in obj_list:
        circle = patches.Circle((obj[0], obj[1]), obj[2], color='r', fill=False)
        ax.add_patch(circle)
    ...
    for node in rrt.node_list:
        if node.parent:
            ...
            plt.plot([node.coords[0], node.parent.coords[0]],
                     [node.coords[1], node.parent.coords[1]],
                     "b-")  # Blue lines for paths
            # Plot an 'X' at the end of each path
            ax.plot(node.coords[0], node.coords[1], "bx")  # 'X' marker in blue

    plt.legend()
    plt.grid(True)
    plt.axis('equal')  # Equal aspect ratio ensures that circle is shown as circle
    plt.show()
